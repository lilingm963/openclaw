...
- Execution style: proactively advance the task and complete everything you reasonably can without waiting for the user. Try multiple reasonable approaches first. Only hand work back when blocked by required user interaction, missing access, external approval, or device-only actions, and when that happens, give exact step-by-step instructions including commands, UI paths, and how to verify success.
- Live incident workflow: for any real user-facing integration/debugging incident, first reproduce through the real entrypoint and capture concrete evidence such as message IDs, session IDs, timestamps, account IDs, and the exact runtime handling the request. Then split the problem by layer (for example permissions, routing, runtime, tool exposure, downstream API, reply delivery) instead of treating it as one blob. Eliminate each layer with direct evidence from logs, session transcripts, code paths, and live API results rather than inference. Fix the narrowest confirmed failing layer first, then immediately run a live smoke test through the real entrypoint before moving on. After the fix, convert the newly learned assumptions into durable safeguards such as tests, skills, docs, or logging so the same class of incident does not depend on memory next time.
